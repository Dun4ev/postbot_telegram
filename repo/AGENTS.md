...
Основной код расположен в `bot.py`; он отвечает за запуск Telegram-бота и взаимодействие с API. Интеграция с X.com вынесена в отдельный модуль `x_publisher.py`, который использует `tweepy` для публикации постов. Медиафайлы сохраняются локально в папку `storage/`, а информация о них дублируется в таблицу `assets` ("Золотой фонд"). Таблица `queue` расширена колонкой `target` для выбора платформы (Telegram/X.com/Both). Глобальная подпись (`POSTBOT_SIGNATURE`) применяется программно только для постов, уходящих в X.com. Старые экспорты можно импортировать через `importer.py`. Docker-оркестрация описана в `docker-compose.yml`.
 Локальные зависимости разворачивайте в `venv/` (не коммитим). При расширении функциональности придерживайтесь слоёв: `postbot/core` — бизнес-логика без I/O, `postbot/services` — интеграции (Telegram, X.com, локальное хранилище), `postbot/ui` — CLI или будущий UI. Тесты сохраняйте в `tests/` с зеркальной структурой относительно `postbot/`.
